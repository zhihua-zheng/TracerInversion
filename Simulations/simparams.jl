...
@with_kw struct SimParams
    commons = (;
               Lz = 150meters,   # depth
               Lf = 1000meters,  # initial frontal width
               ...
               ckp_interval = 1days,    # how often to save checkpoints
               ...
               n_tracers = 4, # number of passive tracers for inversion

               z_refinement = 1.7,  # controls spacing near surface (higher means finer spaced)
               z_stretching = 12,   # controls rate of stretching at bottom
    ...
    DoubleFront = (; commons...,
               Lx = 8008meters, # east-west extent
               Ly = 4004meters, # north-south extent
               Nx_full = 2200, # number of points in the x direction for full simulation
               Ny_full = 1100, # number of points in the y direction for full simulation
               Nz_full = 96,   # number of points in the z direction for full simulation
...
function enrich_parameters(params, casename, spinup, tra_inv)
    if spinup
        ...
        pickup        = false
        pickup_time   = 7days
        pickup_idx    = Int64(pickup_time / params.ckp_interval) + 1 # accounting for day 0
        ...
        save_ckp      = true
        pickup        = true
        pickup_time   = ifelse(M²==3e-8, 6days, 7days)
        pickup_idx    = Int64(pickup_time / params.ckp_interval) + 1 # accounting for day 0
        pickup_prefix = "spinup"
        stop_time     = pickup_time + 2.5days
        if tra_inv
            output_interval = 60minutes
            output_prefix = casename * "_inv-tracer"
            ckp_prefix    = SubString(casename, 21:24) * "_inv-tracer"
            use_fluxed_c  = false
        else
            output_interval = 30minutes
            output_prefix = casename * "_surf-tracer"
            ckp_prefix    = SubString(casename, 21:24) * "_surf-tracer"
            use_fluxed_c  = true
        end
    end
    ...
    Nx = Int(params.Nx_full ÷ (coarsen_h > 1 ? 1.51 : 1))
    Ny = Int(params.Ny_full ÷ (coarsen_h > 1 ? 1.51 : 1))
    Nz = Int(params.Nz_full ÷ (coarsen_z > 1 ? 1.51 : 1))
